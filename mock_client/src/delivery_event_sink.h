...
// Client::HandleWebSocketText, then applies and acknowledges the parsed events.
...
// Records the delivery-relevant events returned by the client codec
...
public:
    ...
    void OnEvent(const ComfyExtensionClient::ClientEvent& event);
    void OnParseError(const ComfyExtensionClient::Error& error);
