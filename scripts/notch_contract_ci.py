...
def runtime_python(runner: CommandRunner) -> Path:
    # The Docker container is the isolation boundary. Installing runtime
    # requirements into the same system Python that owns the baked torch layer
    # lets uv treat torch/torchvision/torchaudio as already satisfied.
    runner.run(["uv", "--version"], log_name="python-install.log")
    runner.run(["uv", "cache", "dir"], log_name="python-install.log")
    return Path(sys.executable)


BAKED_TORCH_REQUIREMENTS = {"torch", "torchvision", "torchaudio"}
...
def requirement_file_for_runtime(
    source: Path,
    workdir: Path,
    skip_requirements: set[str],
) -> Path:
    ...
    filtered_dir = workdir / "filtered-requirements"
    filtered_dir.mkdir(parents=True, exist_ok=True)
    filtered = filtered_dir / f"{source.parent.name}-{source.name}"
    header = [
        "# Generated by notch_contract_ci.py.",
        f"# Omitted baked packages: {', '.join(skipped)}",
    ]
    filtered.write_text("\n".join(header + kept) + "\n", encoding="utf-8")
    return filtered
...
def install_python_deps(
    ...
    if install_torch:
        # Torch is baked into the image layer. Since runtime deps install into
        # the same system Python, skip the multi-GB install when it already
        # imports. Only install if absent — e.g. a base image without the bake
        # layer.
        already = runner.run(
    ...
    skip_requirements = BAKED_TORCH_REQUIREMENTS if install_torch else set()
    if comfy_requirements.is_file():
        requirement_args += [
            "-r",
            str(requirement_file_for_runtime(comfy_requirements, comfy_dir.parent, skip_requirements)),
        ]
    extension_requirements = extension_dir / "requirements.txt"
    if extension_requirements.is_file():
        requirement_args += [
            "-r",
            str(requirement_file_for_runtime(extension_requirements, comfy_dir.parent, skip_requirements)),
        ]
    if requirement_args:
        runner.run(
            ["uv", "pip", "install", "--system", *requirement_args],
            log_name="python-install.log",
        )
...
def python_env_snapshot(runner: CommandRunner, python: Path, artifacts: Path) -> dict[str, Any]:
    freeze = subprocess.run(["uv", "pip", "freeze", "--system"], capture_output=True, text=True, check=False)
    ...
    if freeze.returncode != 0:
        ...
        raise RunnerError("uv pip freeze failed")
    code = (
